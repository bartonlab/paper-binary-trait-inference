...
def find_trait_site(tag,min_n):
    ...
    "store the information for trait sites distance into files (not polymorphic sites between 2 trait sites)"
...
def analyze_result(tag):
    ...
    f.write('polymorphic_index,alignment,HXB2_index,nucleotide,TF,epitope,escape,sc_old,sc_MPL,tc_MPL')
    f.write(',%s' % (','.join(cols)))
    f.write('\n')
    ...
    for ii in range(len(df_poly)):
        ...
        f.write('%s,%s,%s,%f,%f,%s' % (TF, epitope, escape, sc_mpl_old, sc_MPL,tc_MPL))
